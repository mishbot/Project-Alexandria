...
# Properties
#### (*Rayleigh Quotient*) For some real symmetric matrix $A$, $R(A)=\dfrac{x^TAx}{\|x\|^2}$, such that $\lambda_{\max}^{(A)}=\max\limits_{\|x\|=1}R(A)$, and $\lambda_{\min}^{(A)}=\min\limits_{\|x\|=1}R(A)$.
