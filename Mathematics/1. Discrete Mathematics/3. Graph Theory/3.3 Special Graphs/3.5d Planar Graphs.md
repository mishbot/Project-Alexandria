>[!note] Planar Graphs
>A **planar graph** is a graph that can be drawn in the plane without any edges crossing. For any **connected** planar graph, **Euler’s formula** holds, such that
> $$v + f = e + 2,$$
> where $v$ is the number of vertices, $e$ is the number of edges, and $f$ is the number of regions (or **faces**) into which the plane is divided by the graph, including the unbounded exterior face.
# Intuition
...
- In the case of a connected and acyclic graph (a tree), we know that it has $e=v-1$ edges. Since a tree does not form any cycles, $f=1$, and therefore Euler's formula holds.
- In the case of a connected graph with at least one cycle, find a cycle and delete one of the edges. This reduces both $e$ and $f$ by 1, but keeps $v$ constant. Find a cycle and delete one of the edges until the graph becomes a tree. Since Euler's formula holds for trees, by induction **it also holds for all connected planar graphs**.

We see that the core idea of Euler's formula is that for every removed edge that otherwise forms a cycle, the amount of faces decreases by one.

# Properties
##### For any planar graph, $\sum^{f}_{i=1}s_i=2e$.

##### *(Upper Bound of Planarity)* Planar graphs have at most $3v-6$ edges.
