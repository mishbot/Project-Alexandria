...
>A [[4.3 Matrix Multiplication|matrix]] $A \in \mathbb{R}^{m \times n}$ and a vector $\mathbb{R}^{n}$ can be multiplied to create a new vector $\vec{b} \in \mathbb{R}^m$,
>$$
>A \vec{x}=
>\begin{bmatrix}  
...
>\end{bmatrix} = \vec{b},
>$$
>where each element of the resulting vector $b_i = \sum^{n}_{j=1}A_{ij}x_j$ equals the [[3.4b Outer Product|outer product]] of the row vectors of the matrix and the factor vector.
>
